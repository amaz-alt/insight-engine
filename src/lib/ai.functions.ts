...
${(sourcePosts ?? []).map((r) => (r.posts as { content: string } | null)?.content ?? "").join("\n---\n").slice(0, 6000)}`,
    ...
    const rows = output.pieces.slice(0, 12).map((p) => ({
      ...
      kind: data.kinds.includes(p.kind) ? p.kind : (data.kinds[0] ?? "value_post"),
